...
```{r}
Kings <- read_csv2("data/Kongerækken.csv", na = "NA")

remove_cols <- c("...6")

new_df = subset(Kings, select = !(names(Kings) %in% remove_cols)) 

#view updated data frame
new_df

new_df$midyear = new_df$reign_end_date_year - (new_df$reign_end_date_year - new_df$reign_start_date_year)/2 
```
...
new_df <- new_df %>% 
  mutate(midyear =reign_end_date_year)

new_df %>% 
ggplot(aes(x = midyear, y = duration)) +
  geom_smooth() +
  labs(title = "danish Monarchs´ reign", x = "middle year of reign", y = "duration of reign")
```
